...
class Board {
    ...
    PieceState getWinner() {
        ...
        winner = null;
        if (getColorNums(RED) == 0) winner = BLUE;
        if (getColorNums(BLUE) == 0) winner = RED;
		if ((!couldMove(RED) && !couldMove(BLUE)) || getConsecJumpNums() >= 25) {
            if (getColorNums(RED) > getColorNums(BLUE)) winner = RED;
            else if (getColorNums(RED) < getColorNums(BLUE)) winner = BLUE;
            else winner = EMPTY;
        }
